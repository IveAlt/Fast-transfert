...
tk.Label(root, text='Enter IP PC 1 :').grid(row=0, column=0)
...
tk.Label(root, text='Enter IP PC 2 :').grid(row=0, column=2)
...
tk.Checkbutton(root, text='Desktop', variable=Desktop).place(relx=.3, rely=.1)
tk.Checkbutton(root, text='Documents', variable=Documents).place(relx=.3, rely=.2)
tk.Checkbutton(root, text='Downloads', variable=Downloads).place(relx=.3, rely=.3)
...
tk.Checkbutton(root, text='Same User', variable=same_user, command=toggle_user2).place(relx=.0, rely=.4)
...
def long_running_task():
    ...
    with open(log_file_path, 'w') as f:
        ...
        filetrsf_process = subprocess.Popen(
            ['python', 'filetrsf.py', IP1.get(), user.get(), user2.get(), IP2.get(), str(Desktop.get()), str(Documents.get()), str(Downloads.get())],
            stdout=f,
